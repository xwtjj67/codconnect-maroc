...
const MerchantSignup = () => {
  const [form, setForm] = useState({ name: "", username: "", email: "", storeName: "", phone: "", city: "", password: "", confirmPassword: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.name.trim() || !form.username.trim() || !form.email.trim() || !form.storeName.trim() || !form.phone.trim() || !form.city.trim() || !form.password.trim()) {
  ...
  const fields = [
    ...
    { label: "اسم المتجر / العلامة التجارية", key: "storeName", type: "text", placeholder: "اسم المتجر", icon: <Store className="h-4 w-4" /> },
      ...
      <div className="min-h-[80vh] flex items-center justify-center py-12 px-4">
        <div className="w-full max-w-md glass-card p-8 space-y-6">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input type={showPw ? "text" : "password"} placeholder="••••••••" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="w-full h-11 px-10 rounded-lg bg-secondary/50 border border-border text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all" />
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Lock className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <input type={showConfirm ? "text" : "password"} placeholder="••••••••" value={form.confirmPassword} onChange={(e) => setForm({ ...form, confirmPassword: e.target.value })} className="w-full h-11 px-10 rounded-lg bg-secondary/50 border border-border text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all" />
            ...
            <button type="submit" disabled={loading} className="w-full py-3 rounded-xl bg-accent text-accent-foreground font-semibold hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2">
              {loading ? (
                <>
          ...
          <div className="text-center space-y-2 text-sm text-muted-foreground">
            <p>عندك حساب؟ <Link to="/login" className="text-primary hover:underline">تسجيل الدخول</Link></p>
            <p><Link to="/affiliate-signup" className="text-primary hover:underline">سجل كمسوق بدلا من ذلك</Link></p>
          </div>
        </div>
      </div>
    </PublicLayout>
