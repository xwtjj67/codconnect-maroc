...
interface OrderRow { id: string; productName: string; clientName: string; city: string; sellingPrice: number; commissionAmount: number; status: string; createdAt: string; }
...
const AffiliateOrders = () => {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { orders: data } = await api.getMyOrders();
      setOrders(data.map((o: any) => ({ id: o.id, productName: o.product_name || "—", clientName: o.client_name, city: o.city, sellingPrice: Number(o.selling_price), commissionAmount: Number(o.commission_amount), status: o.status, createdAt: new Date(o.created_at).toLocaleDateString("ar-MA") })));
    } catch { }
    setLoading(false);
  }, [user]);
  ...
  return (
    <AffiliateLayout>
      <div className="space-y-6 animate-fade-in">
        <div className="flex items-center justify-between flex-wrap gap-3"><h1 className="text-2xl font-bold">طلباتي</h1><CreateOrderDialog onOrderCreated={fetchOrders} /></div>
        <div className="grid sm:grid-cols-3 gap-3">
          <div className="glass-card p-4 text-center"><p className="text-2xl font-bold text-foreground">{totalOrders}</p><p className="text-xs text-muted-foreground">إجمالي الطلبات</p></div>
          <div className="glass-card p-4 text-center"><p className="text-2xl font-bold text-accent">{confirmedCommissions} DH</p><p className="text-xs text-muted-foreground">عمولات مؤكدة</p></div>
          <div className="glass-card p-4 text-center"><p className="text-2xl font-bold text-primary">{pendingCount}</p><p className="text-xs text-muted-foreground">قيد الانتظار</p></div>
        </div>
