...
const planLimits: Record<string, number> = { standard: 3, premium: 5, vip: -1 };
...
const AffiliateProducts = () => {
  ...
  const maxVisible = planLimits[userPlan] || 3;

  useEffect(() => {
    api.getApprovedProducts().then(({ products: data }) => {
      setProducts(data.map((p: any) => ({ id: p.id, name: p.name, description: p.description, image: p.image, images: p.images || (p.image ? [p.image] : []), videoUrl: p.video_url || null, sellingPrice: p.selling_price ? Number(p.selling_price) : null, commission: p.commission ? Number(p.commission) : null, category: p.category, visibility: p.visibility })));
      setLoading(false);
    }).catch(() => setLoading(false));
  }, []);

  // Filter by category (affiliate's preferred category or selected)
  const filtered = products
    .filter(p => activeCategory === "all" || p.category === activeCategory)
    .filter(p => !search || p.name.includes(search));

  // Apply plan-based limit
  const visibleProducts = maxVisible === -1 ? filtered : filtered.slice(0, maxVisible);
  const hasMore = maxVisible !== -1 && filtered.length > maxVisible;
  ...
  return (
    <AffiliateLayout>
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex flex-wrap gap-2">{CATEGORIES.map(cat => (<button key={cat.value} onClick={() => setActiveCategory(cat.value)} className={`px-4 py-2 rounded-lg text-sm transition-all ${activeCategory === cat.value ? "gradient-teal text-primary-foreground font-medium" : "bg-secondary/50 text-muted-foreground hover:bg-secondary"}`}>{cat.label}</button>))}</div>
          ...
          <>
            ...
            {hasMore && (
              <div className="glass-card p-6 text-center space-y-3">
                ...
                <p className="text-sm text-muted-foreground">لديك {filtered.length - maxVisible} منتجات إضافية. قم بترقية باقتك لرؤية المزيد</p>
                <p className="text-xs text-muted-foreground">Premium = 5 منتجات • VIP = وصول كامل</p>
              </div>
            )}
          </>
        )}
