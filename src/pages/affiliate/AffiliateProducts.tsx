...
const visibilityAccess: Record<string, string[]> = { standard: ["standard"], premium: ["standard", "premium"], vip: ["standard", "premium", "vip"] };
...
const AffiliateProducts = () => {
  ...
  const filtered = products.filter(p => activeCategory === "الكل" || p.category === activeCategory).filter(p => !search || p.name.includes(search));
  ...
  return (
    <AffiliateLayout>
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex flex-wrap gap-2">{categories.map(cat => (<button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-2 rounded-lg text-sm transition-all ${activeCategory === cat ? "gradient-teal text-primary-foreground font-medium" : "bg-secondary/50 text-muted-foreground hover:bg-secondary"}`}>{cat}</button>))}</div>
            ...
            {filtered.map(product => {
              ...
              return (
                <LockedFeature key={product.id} isLocked={isLocked} message="قم بالترقية للوصول لهذا المنتج">
                  <div className="glass-card-hover overflow-hidden cursor-pointer" onClick={() => !isLocked && setSelectedProduct(product)}>
