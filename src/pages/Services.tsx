...
const Services = () => {
  ...
  const handleRequest = async () => {
    if (!name.trim() || !phone.trim() || !role || !selectedService) { toast({ title: "يرجى ملء جميع الحقول", variant: "destructive" }); return; }
    setLoading(true);
    try {
      await api.submitServiceRequest({ name: name.trim(), phone: phone.trim(), role, service_name: selectedService });
      toast({ title: "✅ تم إرسال طلبك بنجاح", description: "سنتواصل معك قريباً" });
      setDialogOpen(false); setName(""); setPhone(""); setRole("");
    ...
    <PublicLayout>
      <div className="min-h-screen py-16" dir="rtl">
        <div className="container max-w-6xl">
          <div className="text-center mb-16"><h1 className="text-4xl md:text-5xl font-bold text-foreground mb-4">خدماتنا</h1><p className="text-lg text-muted-foreground max-w-2xl mx-auto">حلول متكاملة لتنمية أعمالك في التجارة الإلكترونية والتسويق بالعمولة</p></div>
          {categories.map(cat => (
            <section key={cat.title} className="mb-14">
              <div className="flex items-center gap-3 mb-6"><cat.icon className="h-6 w-6 text-primary" /><h2 className="text-2xl font-bold text-foreground">{cat.title}</h2></div>
                ...
                {cat.services.map(svc => (
                  <Card key={svc.title} className="bg-card/80 backdrop-blur-sm border-border/50 hover:border-primary/40 transition-all duration-300 group">
                    <CardContent className="p-6 flex flex-col h-full">
                      <div className="flex items-start justify-between mb-3"><h3 className="text-lg font-semibold text-foreground group-hover:text-primary transition-colors">{svc.title}</h3>{svc.badge && <Badge className={`text-[10px] ${badgeStyles[svc.badge]}`}>{svc.badge}</Badge>}</div>
                      <p className="text-sm text-muted-foreground mb-6 flex-1 leading-relaxed">{svc.description}</p>
                      <div className="flex gap-2"><Button className="flex-1" onClick={() => { setSelectedService(svc.title); setDialogOpen(true); }}>اطلب الخدمة</Button><Button variant="outline" size="icon" onClick={() => openWhatsApp(svc.title)}><MessageCircle className="h-4 w-4" /></Button></div>
                    </CardContent>
                  </Card>
                ))}
              </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md" dir="rtl">
          ...
          <div className="space-y-4 mt-2">
            <div><Label>الاسم الكامل</Label><Input value={name} onChange={e => setName(e.target.value)} placeholder="أدخل اسمك" /></div>
            <div><Label>رقم الهاتف</Label><Input value={phone} onChange={e => setPhone(e.target.value)} placeholder="06XXXXXXXX" /></div>
            <div><Label>الدور</Label><Select value={role} onValueChange={setRole}><SelectTrigger><SelectValue placeholder="اختر دورك" /></SelectTrigger><SelectContent><SelectItem value="affiliate">مسوق بالعمولة</SelectItem><SelectItem value="seller">مورد / بائع</SelectItem></SelectContent></Select></div>
            <div className="flex gap-2 pt-2"><Button className="flex-1" onClick={handleRequest} disabled={loading}>{loading ? "جاري الإرسال..." : "إرسال الطلب"}</Button><Button variant="outline" onClick={() => openWhatsApp(selectedService)} className="gap-2"><MessageCircle className="h-4 w-4" /> واتساب</Button></div>
          </div>
        </DialogContent>
      </Dialog>
    </PublicLayout>
