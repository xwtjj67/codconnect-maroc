...
interface OrderRow {
  id: string; productName: string; merchantName: string; affiliateName: string;
  clientName: string; city: string; sellingPrice: number; commissionAmount: number; status: string;
}
...
const AdminOrders = () => {
  ...
  useEffect(() => {
    api.getAllOrders().then(({ orders: data }) => {
      setOrders(data.map((o: any) => ({
        id: o.id, productName: o.product_name || "—", merchantName: o.merchant_name || "—",
        affiliateName: o.affiliate_name || "—", clientName: o.client_name,
        city: o.city, sellingPrice: Number(o.selling_price),
        commissionAmount: Number(o.commission_amount), status: o.status,
      })));
      setLoading(false);
    ...
    <AdminLayout>
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex items-center gap-2 text-sm flex-wrap">
          ...
          {statusFilters.map(s => (
            <button key={s} onClick={() => setFilter(s)} className={`px-3 py-1.5 rounded-lg transition-all ${filter === s ? "gradient-teal text-primary-foreground font-medium" : "bg-secondary/50 text-muted-foreground hover:bg-secondary"}`}>
        ...
        {loading ? (
          ...
          <div className="glass-card p-12 text-center space-y-3">
            <div className="h-16 w-16 rounded-full bg-secondary/50 flex items-center justify-center mx-auto"><ShoppingCart className="h-8 w-8 text-muted-foreground" /></div>
                ...
                <tbody>
                  {filtered.map(o => (
                    <tr key={o.id} className="border-b border-border/30 last:border-0 hover:bg-secondary/20 transition-colors">
                      ...
                      <td className="p-4"><span className={`px-3 py-1 rounded-full text-xs font-medium ${statusColors[o.status]}`}>{statusLabels[o.status]}</span></td>
                      <td className="p-4">
