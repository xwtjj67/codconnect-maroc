                    ...
                    <tr key={u.id} className="border-b border-border/30 last:border-0 hover:bg-secondary/20 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-1">
                          {u.status !== "active" && <button onClick={() => updateStatus(u.id, "active")} className="p-2 rounded-lg hover:bg-primary/10 text-muted-foreground hover:text-primary transition-colors" title="تفعيل"><CheckCircle className="h-4 w-4" /></button>}
                          {u.status !== "suspended" && <button onClick={() => updateStatus(u.id, "suspended")} className="p-2 rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors" title="إيقاف"><XCircle className="h-4 w-4" /></button>}
                          {u.phone && <button onClick={() => openWhatsApp(u.phone)} className="p-2 rounded-lg hover:bg-primary/10 text-muted-foreground hover:text-primary transition-colors" title="واتساب"><MessageCircle className="h-4 w-4" /></button>}
                        </div>
                      </td>
                    </tr>
                  ))}
